...
contract HubDAO is Ownable, ReentrancyGuard {
    ...
    event BudgetVoted(uint256 indexed quarter, address indexed voter, bool support);
    event BudgetApproved(uint256 indexed quarter, uint256 amount);
    event BudgetExecuted(uint256 indexed quarter, address indexed recipient, uint256 amount);
    event VetoExecuted(uint256 indexed quarter, address indexed vetoer);
    
    constructor(address _treasuryToken, address _initialOwner) Ownable(_initialOwner) {
    ...
    function approveBudget(uint256 quarter) external {
        ...
        require(budget.voteCount >= MIN_VOTING_THRESHOLD / 100, "Insufficient votes");
    ...
    function executeBudget(
        ...
        require(block.timestamp >= budget.startTime, "Budget period not started");
